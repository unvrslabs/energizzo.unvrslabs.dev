...
export function NetworkOverviewStats({ data }: { data: NetworkOverviewData }) {
  const { pendingRequests, invited, inProgress, completed, activeMembers } =
    data;
  ...
  return (
    <div className="space-y-3">
      ...
      <section className="grid grid-cols-1 md:grid-cols-2 gap-3">
        <div className="rounded-2xl border border-primary/25 bg-gradient-to-br from-primary/[0.08] via-white/[0.02] to-transparent backdrop-blur-sm p-4">
        ...
        <div className="rounded-2xl border border-white/10 bg-white/[0.03] backdrop-blur-sm p-4">
          <div className="flex items-center justify-between">
            ...
            <span className="text-xs text-muted-foreground">obiettivo 100</span>
          </div>
          <div className="mt-3 h-1.5 w-full rounded-full bg-white/5 overflow-hidden">
            <div
              className="h-full rounded-full bg-gradient-to-r from-primary to-emerald-500 transition-all"
              style={{
                width: `${Math.min(100, Math.round((activeMembers / 100) * 100))}%`,
              }}
            />
          </div>
        </div>
      ...
      <section className="rounded-2xl border border-white/10 bg-white/[0.03] backdrop-blur-sm p-3">
...
function FunnelStep({
  ...
  return (
    <div
      className={`flex-1 rounded-xl border border-white/10 bg-gradient-to-br ${color} p-2.5 text-center`}
    >
