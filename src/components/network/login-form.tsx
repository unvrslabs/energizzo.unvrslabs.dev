...
export function NetworkLoginForm({ next }: { next?: string }) {
  ...
  async function requestOtp() {
    ...
    setError(null);
    try {
      const res = await fetch("/api/network/auth/request-otp", {
        method: "POST",
  ...
  async function verifyOtp() {
    ...
    setError(null);
    try {
      const res = await fetch("/api/network/auth/verify-otp", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
      ...
      window.location.href = next || data.redirect || "/network";
    } catch (err) {
      setError(err instanceof Error ? err.message : "Errore imprevisto");
    } finally {
      setLoading(false);
    ...
    <div className="relative overflow-hidden rounded-3xl border border-primary/20 bg-gradient-to-br from-primary/[0.06] via-white/[0.02] to-transparent backdrop-blur-sm p-6 md:p-8">
      ...
      <div className="relative">
        ...
        <p className="text-sm text-muted-foreground leading-relaxed mb-6">
          Solo per i membri del network Il Dispaccio. Ti inviamo un codice di
          accesso su WhatsApp.
        </p>
