...
export async function POST(req: NextRequest) {
  ...
  if (!admin) {
    return NextResponse.json(
      { ok: false, error: "Numero non autorizzato." },
      { status: 403 },
    );
  }
  ...
  if ((recentCount ?? 0) >= RATE_MAX_PER_PHONE) {
    return NextResponse.json(
      { ok: false, error: "Troppi codici richiesti. Riprova tra qualche minuto." },
      { status: 429 },
    );
  }
  ...
  if (insertError) {
    console.error("admin otp insert failed", insertError);
    return NextResponse.json(
  ...
  const text = `Il Dispaccio Admin — Codice accesso: ${code}\nValido 5 minuti. Non condividerlo con nessuno.`;
  try {
